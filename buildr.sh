...
# ABA Data Collection App Build Script - RELEASE MODE
# Usage: ./buildr.sh [device-id]
...
if [ -n "$DEVICE_ID" ]; then
    echo "Running on device ID: $DEVICE_ID (RELEASE MODE)"
    ...
    echo ""
    echo "To run on a device, use: ./buildr.sh <device-id>"
